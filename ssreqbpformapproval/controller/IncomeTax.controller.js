sap.ui.define([
	...
	return Controller.extend("com.arteriatech.ssreqbpformapproval.controller.IncomeTax", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.arteriatech.ssreqbpformapproval.view.IncomeTax
		 */
		onInit: function () {
			gITRDocview = this.getView();
		...
		viewAttachemets: function (oEvent) {
			var that = this;
			busyDialog.open();
			var oModelContext = oEvent.getSource().getBindingContext("ITRCertificates");
			var idx = parseInt(oModelContext.getPath().split("/")[1]);
			var Certificates = that.getView().getModel("ITRCertificates").getData();
		...
		getDocuments: function (idx, DocsTypes, gDeleteFlag, callback) {
			...
			var Certificates = gITRDocview.getModel("ITRCertificates").getData();
				...
				success: function (oData) {
					...
					if (oData.results.length > 0) {
						if (DocsTypes) {
							...
							that.getView().getModel("ITRCertificates").setData(Certificates);
							if (that.dialog) {
								that.dialog.close();
							}
