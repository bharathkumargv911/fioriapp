sap.ui.define([
	...
	return Controller.extend("com.arteriatech.zsf.quot.controller.QuotFunctionsView", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.arteriatech.zsf.quot.view.view.QuotFunctionsView
		 */
		onInit: function() {
			this.onInitHookUp();
		},
		onInitHookUp: function() {
			...
			gQuotFunctionsView = this._oView;
			/**
		...
		exportToExcel: function(oEvent) {
			if (Device.system.desktop) {
				oPPCCommon.copyAndApplySortingFilteringFromUITable({
					...
					uiTable: this.getView().byId("UIPartnerFunctionsTable")
				});
			}
			var table = this.getView().byId("PartnerFunctionsTable");
			var oModel = this.getView().getModel("QuotationPartnerFunctions");
