sap.ui.define([
	...
	return Controller.extend("com.arteriatech.ssreqbpformapproval.controller.GSTUpload", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.arteriatech.ssreqbpformapproval.view.GSTUpload
		 */
		onInit: function () {
			gGSTView = this.getView();
		...
		viewAttachemets: function (oEvent) {
			var that = this;
			busyDialog.open();
			var oModelContext = oEvent.getSource().getBindingContext("GSTCertificates");
			var idx = parseInt(oModelContext.getPath().split("/")[1]);
			var Certificates = that.getView().getModel("GSTCertificates").getData();
		...
		getDocuments: function (idx, DocsTypes, gDeleteFlag, callback) {
			...
			var Certificates = gGSTView.getModel("GSTCertificates").getData();
				...
				success: function (oData) {
					...
					if (oData.results.length > 0) {
						if (DocsTypes) {
							...
							that.getView().getModel("GSTCertificates").setData(Certificates);
							if (that.dialog) {
								that.dialog.close();
							}
