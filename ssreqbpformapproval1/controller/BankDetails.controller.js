sap.ui.define([
	...
	return Controller.extend("com.arteriatech.ssreqbpformapproval.controller.BankDetails", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.arteriatech.ssreqbpformapproval.view.BankDetails
		 */
		onInit: function () {
			gBankDataview = this.getView();
		...
		viewAttachemets: function (oEvent) {
			var that = this;
			busyDialog.open();
			var oModelContext = oEvent.getSource().getBindingContext("BankCertificates");
			var idx = parseInt(oModelContext.getPath().split("/")[1]);
			var Certificates = that.getView().getModel("BankCertificates").getData();
		...
		getDocuments: function (idx, DocsTypes, gDeleteFlag, callback) {
			...
			var Certificates = gBankDataview.getModel("BankCertificates").getData();
				...
				success: function (oData) {
					...
					if (oData.results.length > 0) {
						if (DocsTypes) {
							...
							that.getView().getModel("BankCertificates").setData(Certificates);
							if (that.dialog) {
								that.dialog.close();
							}
